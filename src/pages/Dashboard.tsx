...
import { Typography, Row, Col, Card, Select, Radio, Input, Progress, Space, Button, Spin, Empty, Tooltip, Modal, Table, message } from 'antd'; // Adicionado message
...
import { getStudentsPredictions, getAllStudents, uploadStudentsCsv } from '../services/StudentService'; // Adicionado uploadStudentsCsv
...
const DashboardPage: React.FC = () => {
  ...
  // --- Função de Carregamento de Dados (Extraída para reutilização) ---
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setUploading(true); // <--- INICIA O LOADING

      try {
        await uploadStudentsCsv(file);

        message.success(`Arquivo "${file.name}" importado com sucesso!`);
        setIsModalOpen(false); // Fecha o modal só no sucesso
        loadDashboardData();

      } catch (error) {
        message.error('Erro ao importar arquivo. Verifique o formato e tente novamente.');
        console.error(error);
      } finally {
        setUploading(false); // <--- PARA O LOADING
        event.target.value = '';
      }
    }
  };
  ...
  return (
    <AppLayout>
      <Spin 
        spinning={uploading} 
        fullscreen 
        tip="Enviando dados para a Inteligência Artificial..." 
        size="large"
      />
      {/* HEADER DA PÁGINA */}
      ...
      <Modal
        ...
        centered
        width={800}
        // ATUALIZE O FOOTER AQUI:
        footer={[
          <Button
            key="back"
            onClick={handleCancelModal}
            disabled={uploading} // Desabilita cancelar durante upload
          ...
          <Button
            ...
            loading={uploading} // Adiciona o spinner de carregamento
            disabled={uploading}
